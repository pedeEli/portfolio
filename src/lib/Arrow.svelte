<script lang="ts">
	...
	let { side, bgColor, href }: Props = $props()

	const getPosition = (s: typeof side) => {
		if (s === 0) return 'bottom-3 top-auto'
		if (s === 1) return 'left-3 right-auto'
		if (s === 2) return 'top-3 bottom-auto'
		return 'right-3 left-auto'
	}
</script>

<div
	class="absolute text-xl inset-0 {getPosition(side)} group flex justify-center {side % 2 === 0
		? 'flex-row'
		: 'flex-col'}"
>
	<a
