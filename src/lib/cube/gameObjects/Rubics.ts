import {Cube, Transform, rotationFirst} from '.'
...
export class Rubics implements GameObject {
    private _cubes: Cube[][][] = []
    ...
    public getPlane(axis: Cube.Axis, index: number) {
        if (axis === 'x')
    ...
    public turn(axis: Cube.Axis, index: number, angle: number) {
        if (axis === 'x')
